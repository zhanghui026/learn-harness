...
课程按「疼痛驱动」推进：每一课先摆出一个具体做不到的场景，再补上解决它的那一件事——所以每个抽象在被命名之前，你已经先欠过它的债。下面的文件表就是这条债务链。

**课程正文在 [`course/`](course/)**：一个自包含的双语 HTML，克隆下来用浏览器打开 `course/agent-harness-course.html` 即可（GitHub 不渲染仓库里的 HTML）。讲义共 **13 课（L0–L12）**；下表的 12 个文件是 L1–L11 的代码（L7 占两个），L0 和 L12 没有代码，只存在于讲义里。讲义里每一段 transcript 都是这里的代码真跑出来的输出。
...
仓库根目录的 `.env` 是练习用的假密钥文件，已被 `.gitignore` 忽略；真的 key 走 `export`，从不落盘。
...
`l10-harness.mjs` 现在**能写文件**：`writePolicy`（判定）+ `approval`（审批，ask/never/always）+ `fsWrite`（工具与沙箱模式栅栏）三个插件，循环一行没改。写入默认走 `read-only` -> 问人 -> 只放宽这一次；`--yes` 一律批准，`--policy never` 一律拒绝，没有 TTY 也失败关死。

`fixtures/` 是快照测试的夹具目录，**禁止写入**（`writePolicy` 里的一条规则），**内容也不许改**（改了 `l9-testing.mjs` 的快照立刻红）——这条规则就是被快照抓出来之后补的。演示产物写到 `out/`；`snapshots/replay.txt` 是录好的 transcript，随仓库提交。

## 每课的开关

- **L5** 用 macOS 的 `sandbox-exec`（seatbelt）；Linux 上把 `runConfined` 换成 `bwrap`/landlock，其余不动。
- **L6** 的三个阈值都走环境变量：`COMPACT_AT=6000 node l6-context.mjs "…"` 逼它压缩给你看；`SPILL_CHARS`、`MAX_STEPS`、`KEEP_RECENT_STEPS` 同理。
- **L7** 的三种组合：默认 `local`+`deepseek`；`--world subprocess` 换到另一个进程（pid 会变）；`--llm replay --from runs/<id>.jsonl` 零成本回放（工具仍然真跑，只有模型那侧被顶替）。
- **L8**：`--without <插件名>` 少装一个、`--trace` 打印每一次派发、`--prove-dispose` 证明卸载真的把注册撤干净了。
...
日志在 `runs/<id>.jsonl`（`runs/` 不进版本库），一行一个事件，只追加不改写。想确认「模型历史是算出来的」，把某个 `tool/result` 行删掉再 `--resume`，模型会重新去读那个文件——它是真的不记得了。

## 每课的练习

- **L2**：造一个假 `.env` 让它念出来，再加一条守卫拒绝读凭据文件，看模型收到的是结果不是异常；然后 `TOOL_TIMEOUT_MS=1000` 重跑一个 `sleep(2)` 的任务，看合法的慢被误伤。
- **L3**：跑一个要好几步的任务，中途打字插一句「先别管那个，先看 README」，观察它在哪一刻被接住；再按 Ctrl-C，看进程是不是还活着。
- **L4**：让它读两个文件，`cp` 一份日志出来删掉其中一个 `tool/result`，`--resume` 那份问被删掉的内容；对照组直接答，动过手的重新去读。也可以从中间 `--fork` 出两条方向，比较两个 jsonl 的公共前缀。
...
- **L10**：用六个问题去审一个真实 agent 项目；把最早和最晚的日志 `--show` 出来对比。
...
- **课后**：`write_file` 已经在 `l10-harness.mjs` 里实现了（见上文），把五种情况亲手各验一遍——新建要升级、覆盖要审批、`.env` 直接拒、越界直接拒、无人值守一律拒。
